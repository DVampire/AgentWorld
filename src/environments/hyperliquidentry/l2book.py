"""L2 order book data handler for Hyperliquid streaming data."""
...
class L2BookHandler:
    """Handler for L2 order book data with streaming and caching."""
    
    def __init__(self, database_service: DatabaseService):
        """Initialize L2 book handler.
    ...
    async def ensure_table_exists(self, symbol: str) -> None:
        """Ensure l2book table exists for a symbol.
        
        Args:
            symbol: Symbol name
        """
        base_name = self._sanitize_table_name(symbol)
        table_name = f"{base_name}_l2book"
        ...
        columns = [
            ...
            {"name": "best_bid_price", "type": "REAL"},
            {"name": "best_bid_size", "type": "REAL"},
            {"name": "best_ask_price", "type": "REAL"},
            {"name": "best_ask_size", "type": "REAL"},
        ...
        if not result.success:
            logger.error(f"Failed to create l2book table {table_name}: {result.message}")
            raise HyperliquidError(f"Failed to create l2book table {table_name}: {result.message}")
    ...
    async def stream_insert(self, data: Dict, symbol: str) -> bool:
        """Insert L2 book data from stream.
        
        Args:
            data: Processed L2 book data dictionary from Hyperliquid WebSocket stream
            symbol: Symbol name
        ...
        try:
            ...
            #   "bids": List[Dict],  # List of {px: str, sz: str, n: int}
            #   "asks": List[Dict],  # List of {px: str, sz: str, n: int}
            ...
            logger.debug(f"| 🔍 Attempting to insert l2book data for {symbol} into {table_name}")
            
            result = await self.database_service.insert_data(insert_request)
            
            if not result.success:
                logger.error(f"| ❌ Failed to insert l2book data for {symbol}: {result.message}")
                logger.error(f"| ❌ Insert request: table={table_name}")
                return False
            ...
            if verify_result.success:
                ...
                logger.debug(f"| ✅ L2Book data inserted for {symbol}. Total rows in {table_name}: {count}")
            else:
            ...
            logger.error(f"Error inserting l2book data for {symbol}: {e}")
            return False
    ...
    async def get_data(
        ...
        """Get L2 book data from database.
        ...
        Returns:
            List of L2 book records
        """
        base_name = self._sanitize_table_name(symbol)
        table_name = f"{base_name}_l2book"
        
        # Check if l2book table exists
        check_query = f"SELECT name FROM sqlite_master WHERE type='table' AND name='{table_name}'"
        check_result = await self.database_service.execute_query(
            QueryRequest(query=check_query)
        )
        ...
        if not check_result.success or not check_result.extra.get("data"):
            logger.warning(f"| ⚠️  L2Book table {table_name} does not exist for {symbol}")
            return []
        
        # Build query for l2book based on whether date range is provided
        ...
        if not result.success:
            logger.warning(f"| ⚠️  Failed to query l2book from {table_name}: {result.message}")
            return []
        
        l2book_data = result.extra.get("data", [])
        logger.debug(f"| 🔍 Query returned {len(l2book_data)} rows for {symbol}")
        
        if l2book_data:
            logger.debug(f"| 🔍 First row sample: {l2book_data[0]}")
        else:
            logger.warning(f"| ⚠️  No l2book data returned for {symbol} from table {table_name}")
        ...
        for record in l2book_data:
            if record.get("bids"):
                try:
                    record["bids"] = json.loads(record["bids"])
                except (json.JSONDecodeError, TypeError):
                    record["bids"] = []
            if record.get("asks"):
                try:
                    record["asks"] = json.loads(record["asks"])
                except (json.JSONDecodeError, TypeError):
                    record["asks"] = []
