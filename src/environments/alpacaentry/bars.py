...
class BarsHandler:
    ...
    def __init__(self, database_service: DatabaseService):
        ...
        self._cache_limit: int = 200  # Maximum number of bars to cache per symbol
    ...
    async def ensure_table_exists(self, symbol: str) -> None:
        """Ensure bars table exists for a symbol.
        ...
        if not result.success:
            logger.error(f"Failed to create bars table {table_name}: {result.message}")
            raise AlpacaError(f"Failed to create bars table {table_name}: {result.message}")
    ...
    async def ensure_indicators_table_exists(self, symbol: str) -> None:
        """Ensure technical indicators table exists for a symbol.
        ...
        result = await self.database_service.create_table(create_request)
        if not result.success:
            logger.error(f"Failed to create indicators table {table_name}: {result.message}")
            raise AlpacaError(f"Failed to create indicators table {table_name}: {result.message}")
    ...
    def _prepare_data_for_insert(self, data: Dict, symbol: str) -> Dict:
        ...
        if 'timestamp' in data:
            ...
            if hasattr(timestamp_value, 'strftime'):
                ...
                # Assume it's a formatted string already
    ...
    async def stream_insert(self, data: Dict, symbol: str) -> bool:
        ...
        Args:
            data: Raw bars data from Alpaca stream
            symbol: Symbol name
    ...
    def calculate_indicators(self, bars: Union[List[Dict], pd.DataFrame]) -> Optional[Dict]:
        """Calculate technical indicators from bars data.
        
        Args:
            bars: List of bars records or DataFrame with fields: open, high, low, close, volume, timestamp
        ...
        try:
            closes = np.array([float(b.get("close", 0)) for b in bars_list if b.get("close") is not None], dtype=np.float64)
            highs = np.array([float(b.get("high", 0)) for b in bars_list if b.get("high") is not None], dtype=np.float64)
            lows = np.array([float(b.get("low", 0)) for b in bars_list if b.get("low") is not None], dtype=np.float64)
            opens = np.array([float(b.get("open", 0)) for b in bars_list if b.get("open") is not None], dtype=np.float64)
            volumes = np.array([float(b.get("volume", 0)) if b.get("volume") is not None else 0.0 for b in bars_list], dtype=np.float64)
            
            if len(closes) != len(bars_list) or len(closes) == 0:
                return None
    ...
    async def _calculate_and_store_indicators(self, symbol: str) -> None:
        ...
        try:
            ...
            await self.ensure_indicators_table_exists(symbol)
    ...
    async def get_data(
        ...
        if not result.success:
            logger.warning(f"Failed to query bars from {table_name}: {result.message}")
            return []
    ...
    async def get_indicators(
        ...
        Returns:
            List of indicator records, each with a 'indicators' dict containing only non-NULL values
        """
        base_name = self._sanitize_table_name(symbol)
        table_name = f"{base_name}_indicators"
        ...
        if not result.success:
            logger.warning(f"Failed to query indicators from {table_name}: {result.message}")
            return []
        
        indicator_records = result.extra.get("data", [])
        
        # Process records: convert to dict with only non-NULL indicator values
        processed_records = []
        for record in indicator_records:
            ...
            indicator_fields = [
                ...
                "macd", "macd_signal", "macd_hist", "adx", "sar",
            ...
            for field in indicator_fields:
                ...
                if value is not None:
                    processed_record["indicators"][field] = value
